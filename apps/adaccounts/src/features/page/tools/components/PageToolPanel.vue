...
const { orderedFunctions, selectedFunctionIds, selectFunction } = pageToolActions;
...
function buildGroups(): ToolGroup[] {
  const midpoint = Math.ceil(orderedFunctions.value.length / 2);
  return [
    { id: 'group-1', label: 'Group 1', functions: orderedFunctions.value.slice(0, midpoint) },
    { id: 'group-2', label: 'Group 2', functions: orderedFunctions.value.slice(midpoint) },
  ];
}
...
const filteredGroups = computed(() => {
  ...
  return groups.value
    .map((group) => ({
      ...group,
      functions: group.functions.filter((fn) => `${group.label} ${fn.label}`.toLowerCase().includes(query)),
    }))
...
<template>
  <div class="flex h-full min-h-0 flex-col overflow-hidden rounded-[16px] bg-transparent text-black">
    <div class="flex items-center justify-between px-5 py-4">
      <div>
        <h2 class="text-sm font-normal tracking-[-0.02em] text-black">Kho chức năng Page</h2>
      </div>
    ...
    <div class="flex items-center gap-2 px-5 pb-4">
      ...
      <button type="button" disabled class="grid size-8 shrink-0 cursor-not-allowed place-items-center rounded-full bg-white/60 text-black/35" title="Lịch sử thao tác — chưa hỗ trợ">
        <Icon name="clock" :size="12" />
      </button>
      <button type="button" disabled class="grid size-8 shrink-0 cursor-not-allowed place-items-center rounded-full bg-white/60 text-black/35" title="Chức năng đã ghim hiển thị trên từng dòng">
    ...
    <div class="min-h-0 flex-1 overflow-auto px-5 pb-5">
      <div v-if="filteredGroups.length > 0" class="space-y-3">
        <section v-for="group in filteredGroups" :key="group.id" class="overflow-hidden rounded-[20px] bg-white text-black shadow-[0_16px_32px_0_rgba(0,177,115,0.06)]">
          <button type="button" class="flex w-full items-center gap-3 px-3 py-3 text-left transition-colors hover:bg-black/[0.03]" @click="toggleGroup(group.id)">
            ...
            <Icon name="chevron-down" :size="16" class="shrink-0 text-black/25 transition-transform" :class="isGroupOpen(group.id) ? 'rotate-180' : ''" />
          </button>

          <div v-if="isGroupOpen(group.id)" class="border-t border-black/[0.06]">
            <draggable v-if="!normalizedKeyword" v-model="group.functions" item-key="id" handle=".page-tool-drag-handle" :animation="160" ghost-class="opacity-40">
              <template #item="{ element: fn }">
                ...
                >
                  <span class="page-tool-drag-handle -ml-1 grid size-7 shrink-0 cursor-grab place-items-center text-black/25 active:cursor-grabbing" @click.stop>
                  ...
                  <button type="button" class="grid size-6 shrink-0 place-items-center rounded-full text-black/35 transition hover:bg-emerald-50 hover:text-emerald-600" :class="isPinned(fn.id) ? 'text-emerald-600' : ''" :title="isPinned(fn.id) ? 'Bỏ ghim' : 'Ghim chức năng'" @click.stop="togglePinned(fn.id)">
